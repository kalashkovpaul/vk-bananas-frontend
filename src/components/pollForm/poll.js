...
export default class Poll extends Component {
  ...
  render() {
    ...
    return (
      <article className={`animate__animated animate__fadeIn animate__faster animate__poll`} style={{ textAlign: customStyles.align, alignItems: this.alignPoll(customStyles.align) }}>
        ...
        <ul className="answers">
          {answers.map(answer => (
            <li key={answer.option}>
              {!poll.voted ? (
                <button className={`animate__animated animate__fadeIn animate__faster option ${customStyles.theme}`} style={{ color: colors[0], borderColor: colors[1] }} type='button' onClick={() => this.vote(answer.option)}>
                  {answer.option}
                </button>
              ) : (
                <div className={`animate__animated animate__fadeIn animate__faster result`} style={{ color: colors[0], borderColor: colors[1] }}>
                  <div className="fill" style={{ width: this.calculatePercent(answer.votes, totalVotes), backgroundColor: colors[2] }} />
                  <div className="labels">
                    <span className="percent" style={{ color: colors[0] }}>{this.calculatePercent(answer.votes, totalVotes)}</span>
                    <span className={`$"answer" ${"option" === poll.option ? "vote" : ''}`} style={{ color: colors[0] }}>{answer.option}</span>
                  </div>
                </div>
              )}
            </li>
